...
const CRUISE = 42;
const STAGGER = 0.4;
const HOVER_SETTLE = 0.55;
const POD_GAP = 0.16;
const POD_G = 26;
const LINGER_S = 6.4;
const SPAWN_WAIT = 0.22;
const LIVE_MAX = 8;
...
const GEO = {
  ...
  pod: new THREE.CapsuleGeometry(0.2, 0.62, 3, 6),
  pip: new THREE.SphereGeometry(0.045, 6, 4),
  dust: new THREE.RingGeometry(0.32, 0.62, 12),
};
...
function makeDust() {
  const m = new THREE.Mesh(GEO.dust, MAT.dust);
...
export function spawnDropships(scene, opts = {}) {
  ...
  function hatchPose(c, open) {
    ...
    hatches[0].position.y = -0.4 - open * 0.04;
    hatches[1].position.y = -0.4 - open * 0.04;
  }
  ...
  function fly(c, dt) {
    ...
    if (c.mode === "inbound") {
      ...
      c.bank += ((u > 0.55 ? 0 : Math.sign(c.heading || 0.4) * 0.28) - c.bank) * Math.min(1, dt * 2.2);
      c.pitch += (-0.16 * (1 - u) - c.pitch) * Math.min(1, dt * 2);
      ...
      c.pos.y += (HOVER_Y + Math.sin(pulseT * 1.7 + c.idx) * 0.2 - c.pos.y) * Math.min(1, dt * 1.8);
      c.bank += (0 - c.bank) * Math.min(1, dt * 2.4);
      c.pitch += (0.03 - c.pitch) * Math.min(1, dt * 2);
      ...
      c.pos.y += (HOVER_Y + Math.sin(pulseT * 1.5 + c.idx) * 0.22 - c.pos.y) * Math.min(1, dt * 1.5);
      c.bank += (0.12 - c.bank) * Math.min(1, dt * 1.6);
      ...
      c.bank += ((c.idx % 2 === 0 ? 0.38 : -0.38) - c.bank) * Math.min(1, dt * 1.8);
      c.pitch += (-0.2 - c.pitch) * Math.min(1, dt * 1.6);
      const gone =
        c.pos.y > 26 ||
  ...
  function tickPods(dt) {
    for (let i = pods.length - 1; i >= 0; i--) {
      ...
      if (!p.landed) {
        ...
        p.root.rotation.x = Math.min(0.35, -p.vy * 0.02);
      } else {
        p.wait -= dt;
        p.root.position.y = 0.02;
        if (p.wait <= 0) queueSpawn(p);
        p.root.scale.setScalar(Math.max(0.01, 1 - Math.max(0, -p.wait) * 3.5));
      ...
      if (p.dustT > 0) {
        p.dustT -= dt;
        p.dust.scale.setScalar(1 + (0.28 - p.dustT) * 3);
  ...
  function tick(dt, playerPos) {
    ...
    for (const c of craft) {
      ...
      if (ex) {
        const s = c.mode === "peel" ? 1.25 : c.mode === "inbound" ? 1.1 : 0.85;
        for (const m of ex) m.scale.set(s, s * (0.8 + pulse), s);
      }
    }
    tickPods(h);
    tryFlush();
  ...
  function dispose() {
    for (const c of craft) scene?.remove(c.root);
    craft.length = 0;
    for (const p of pods) {
      scene?.remove(p.root);
      scene?.remove(p.dust);
